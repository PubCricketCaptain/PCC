...
// Multiplayer disabled for testing
let mp = {active:false,role:null,roomCode:null,unsubscribe:null,lastUpdate:null,dropTimer:null,actionTimer:null};
function mpRoomPath(c){return 'games/'+c;}
function mpCreateRoom(){alert('MP disabled');}
function mpJoinRoom(c){alert('MP disabled');}
function mpLeave(){render();}
function mpPush(){}
function mpSubscribe(){}
function mpGetScreen(){return st.gameScreen;}
function mpIsHost(){return true;}
function mpIsGuest(){return false;}
function mpClearActionTimer(){}
function mpStartActionTimer(){}
function mpStartDropDetection(){}
function mpEndWithoutPenalty(){mpLeave();}
function mpForfeit(){if(confirm('Forfeit?'))resetGame();}
function mpAdvanceToConditions(){}
function mpAdvanceToTeams(){}
function mpAdvanceToToss(){}
function mpSelectTeamIdx(){}
function mpSelectTeam(){}
function mpCallToss(c){tossTheCoin();}
function renderMpPhase(){return renderSelectTeams();}
function renderMpLobby(){return '';}
function renderMpConditions(){return '';}
function renderMpTeams(){return '';}
function renderMpToss(){return '';}
...
// ═══════════════════════════════════════
const PITCH_OPTS = [
  {id:'minefield',label:'Minefield',icon:'⚡',desc:"Seaming, crumbling — bowlers' paradise", idx:0, weight:1},
  {id:'good',     label:'Good',     icon:'🌿',desc:'Fair contest, true bounce',             idx:1, weight:4},
  {id:'flat',     label:'Flat Track',icon:'🏏',desc:"Batter's highway — high scores expected",idx:2, weight:2},
];
const WEATHER_OPTS = [
  {id:'sunny',   label:'Sunny',   icon:'🌞',desc:'Standard conditions',                    idx:0, weight:5},
  {id:'overcast',label:'Overcast',icon:'☁️', desc:'Helps swing and seam bowlers',           idx:1, weight:3},
  {id:'hot',     label:'Hot',     icon:'🔆',desc:'Ball bounces — fast bowlers thrive',      idx:2, weight:2},
  {id:'damp',    label:'Damp',    icon:'🌧️',desc:'Slow, low scoring — tough for all',       idx:3, weight:1},
];
...
const MENTALITIES = [
  {id:'defensive',   label:'Defensive',   icon:'🛡️', desc:'Dot balls, hard to dismiss'},
  {id:'rotation',    label:'Rotate',      icon:'🔄', desc:'Singles & twos, keeps strike'},
  {id:'positive',    label:'Positive',    icon:'✅', desc:'Balanced approach'},
  {id:'aggressive',  label:'Aggressive',  icon:'💥', desc:'Boundary hunting, higher risk'},
];
...
function dismissBatsman(dtype, logPrefix){
  ...
  if(st.bowler){
    ...
    if(consec===3) addLog(`🎩 HAT TRICK! ${bwlName} — three in a row!`,'boundary');
    else if(consec===4) addLog(`🎩🎩 DOUBLE HAT TRICK! ${bwlName} — four in a row!`,'boundary');
    else if(consec>=5){
      const names=['','','','','Triple','Quadruple','Quintuple','Sextuple'];
      const n=names[consec]||`${consec}-wicket`;
      addLog(`🎩 ${n.toUpperCase()} HAT TRICK! ${bwlName} — ${consec} in a row!`,'boundary');
    }
    // 5-wicket milestone
...
function selectField(id){
  if(!st.done&&!st.matchOver&&!st.pendingDismissal){
    if(st.field!==id){
      const icons={attacking:'⚔️',balanced:'⚖️',defensive:'🛡️'};
...
function renderToss(){
  ...
  return`<div class="setup">
    ...
    `:`
    <div class="coin" id="coin" onclick="tossTheCoin()">🪙\u003C/div>
...
<div style="margin-top:10px;display:flex;gap:8px;justify-content:center">
  <button class="btn sec" style="font-size:11px;padding:4px 10px" onclick="copyTranscript()">📋 Transcript\u003C/button>
  ${mp&&mp.active
...
<div class="pending-box">
  <div class="pending-title">⚖️ ${getDismissalFlavour(bat?.runs||0)} — ${st.pendingDismissal.dtype}\u003C/div>
...
function renderPendingWicket(){
  if(isCpuBatting()){
    ...
    return`<div class="pending-box" style="border-color:var(--red);background:var(--red-pale)">
      <div class="pending-title" style="color:var(--red)">🏏 Wicket!\u003C/div>
      <div style="font-size:11px;color:var(--mid);font-style:italic;margin-top:4px">Next batsman coming in…\u003C/div>
    \u003C/div>`;
  }
  return`<div class="pending-box" style="border-color:var(--red);background:var(--red-pale)">
  <div class="pending-title" style="color:var(--red)">🏏 Wicket! Who bats next?\u003C/div>
...
<div class="result-banner ${result.cls}">
  ...
  <div style="margin-top:10px;display:flex;gap:8px;justify-content:center;flex-wrap:wrap">
    ...
    <button class="btn btn-sm" style="background:var(--mid)" onclick="copyTranscript()">📋 Transcript\u003C/button>
  \u003C/div>
\u003C/div>`;
}
...
<div class="cond-bar">
  ...
  <div style="flex:1">\u003C/div>
  <button class="btn btn-sm" onclick="openSettings()" style="margin-right:4px">⚙️\u003C/button>
  <button class="btn btn-sm" onclick="copyTranscript()" style="margin-right:4px" title="Copy match transcript">📋\u003C/button>
...
<div class="pending-box" style="border-color:var(--red);background:var(--red-pale)">
  <div class="pending-title" style="color:var(--red)">🏏 Wicket! Who&#39;s coming in?\u003C/div>
  <div class="pending-btns" style="margin-top:8px;flex-wrap:wrap">
...
${st.pendingDismissal&&st.pendingDismissal.givenOut?`
<div class="pending-box">
  <div class="pending-title">⚖️ ${getDismissalFlavour(bat.runs)} — ${st.pendingDismissal.dtype}\u003C/div>
  ...
  return`<div class="te-overlay">
    <div class="te-header">
      ...
      <div style="display:flex;gap:6px;align-items:center">
        ...
        <button class="btn btn-sm btn-gold" onclick="exportStatsXlsx()" ${!matchHistory.length?'disabled':''}>↓ xlsx\u003C/button>
      \u003C/div>
    \u003C/div>
...
function renderSelectTeams(){
  ...
  return`<div>
    ...
    <div style="margin-top:14px;border-top:1px solid var(--border);padding-top:14px">
      <div class="card-title" style="margin-bottom:8px">📡 Online Multiplayer\u003C/div>
      ${mp&&mp.active?`
...
function renderTeamEditor(){
  ...
  return`<div class="te-overlay">
    ...
    <div class="profile-tabs" style="margin-bottom:12px">
      <div class="profile-tab${teTab==='batting'?' active':''}" onclick="switchTeTab('batting')">🏏 Batting Order\u003C/div>
      <div class="profile-tab${teTab==='bowling'?' active':''}" onclick="switchTeTab('bowling')">🎯 Bowling\u003C/div>
    \u003C/div>
...
function runSimulation(){
  ...
  showToast(`⚡ ${simCount} matches simulated · avg ${avg}/${avgWkt}`);
...
window.setMentality=(idx,m)=>{
  ...
  const icons={defensive:'🛡️',rotation:'🔄',positive:'✅',aggressive:'💥'};
