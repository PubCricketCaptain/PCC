...
## Workflow Rules
- **Always plan before implementing.** Claude must propose and confirm an implementation plan with the user before writing any code.
- **Always build and run the sim before any balance, CPU logic, or engine changes.** 5000 games, all 11 conditions. Propose changes → validate in sim → ship. Gut-feel tuning without sim validation is how balance bugs creep in.
...
- CPU bowler name shown on bowling screen before Umpire signals play (bug fixed session 6)
- CPU field set correctly on first render (bug fixed session 6)
...
- CRR shown on scoreboard at all times (once a ball bowled); RRR shown in innings 2
- Par score table (11 conditions, from sim) baked in — used by CPU field logic
...
**CPU difficulty:**
- Easy 🍺: cpuSelectMentality / cpuSelectField — makes suboptimal captain decisions that give the human a way back in. Leans passive when fielding if human is behind par; slightly reckless when batting.
- Hard 🧠: smartHumanMentality / smartHumanMentalityChase / smartHumanField — plays correct cricket. Par-aware field decisions in innings 1. Banks wickets early, explodes in death overs.

**Hard CPU batting philosophy:**
- *Setting (inn 1):* Conservative early (rotation/defensive overs 1-3), build in middle (positive overs 4-7), aggressive in death if wickets in hand.
- *Chasing (inn 2):* Required rate gates aggression. Bat sensibly if RRR is comfortable — don't slog a manageable chase. Only open up when RRR demands it or death overs with wickets to spare. If a burst of sixes wins it early, fine — no artificial pacing.

**Hard CPU bowling philosophy:**
- Par-aware field selection in innings 1. If human is ahead of par → attacking (take wickets). If behind par → patient (balanced/defensive, let them keep struggling). No engine buffs/debuffs — only field and bowler selection decisions.
- Innings 2 chase: field driven by required rate as before.
...
- `PAR_SCORES` object: 11 condition keys (`pitchId_weatherId`) → expected first innings total, from sim data. Present in code but **not yet wired to field logic** — sim validation showed par-aware field selection broke balance. Needs a proper redesign next session.
- `getRunsAheadOfPar()`: linear interpolation of par at any ball, returns signed delta. Ready to use once field logic redesign is sim-validated.
- **`sim.js`**: standalone Node.js sim, lives in repo alongside `index.html`. Run with `node sim.js`. Runs 5000 games × 11 conditions, outputs Inn1 avg, Bats1st%, LastOver% for both baseline and any variant. Always run this before shipping balance or CPU logic changes.

---

### Pending Tasks (Priority Order)

1. **Hard CPU mentality retuning** — banking wickets early / exploding in death not yet sim-validated. Needs sim run before touching thresholds. See CPU batting philosophy above for intent.
2. **Easy CPU field/mentality** — suboptimal captain logic not yet fully implemented (currently same as Hard in places). Needs plan + sim validation.
3. **CPU field spam cooldown** — Hard mode helps but Easy CPU still flips field too often. Add a minimum balls-between-changes threshold.
4. **MP end-to-end test** — Firebase MP restored and guest guard added but not fully tested host/guest full match.
5. **Good/overcast win% 62%** — slightly high. Small howzat trim would bring it to ~58%. Low priority.
6. **Minefield personality exploit re-sim** — re-run Setting vs Balanced on minefield now that MP flow fix is in.
...
**Always rebuild and run the sim before any balance, CPU logic, or engine changes.** The sim is the source of truth — gut-feel tuning without it is how balance bugs creep in.

Rebuild from `index.html` using the extraction pattern. Key files: `engine_patched.js`, `smart_batting.js`, `smart_fielding.js`, `sim_runner.js`. Run with Node.js v18+, no npm needed.

Smart batting/fielding functions live in the main game script — extract from there rather than writing separately.

Target: **5000 games across all 11 conditions.** Compare Hard CPU win% and average scores before vs after any changes.
...
- **Session 6:** CPU bowler interstitial bug fixed (bowler/field now applied at render time, not on button press); CRR added to scoreboard (RRR shown in innings 2); PAR_SCORES table + getRunsAheadOfPar() added (not yet wired to field logic — sim showed par-aware field broke balance badly); smartHumanField and cpuSelectField reverted to original validated logic; sim.js built and added to repo alongside index.html; workflow rules added (plan-first, sim-first)
